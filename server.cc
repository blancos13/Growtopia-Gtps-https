...
int main(void) {
  std::cout << "HTTPS Web Server is running with details 127.0.0.1:443 Port 17095 - Altanb#3535" << std::endl;
  ...
  svr.Post(("/growtopia/server_data.php"), [](const Request& req, Response& res) {
          svr.set_connection_timeout(1000);
          svr.set_read_timeout(1000);
          std::string ip = req.remote_addr;
          cooldown.insert({ ip, (GetCurrentTimeInternalSeconds() + (2)) });
  ...
  svr.Get("/cache/game/", [](const Request& /*req*/, Response& res) {
      });
  svr.Get("/cache/social/", [](const Request& /*req*/, Response& res) {
      });
  svr.Get("/cache/interface/", [](const Request& /*req*/, Response& res) {
      });
  svr.Get("/cache/audio", [](const Request& /*req*/, Response& res) {
      });
